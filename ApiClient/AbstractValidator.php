    ...
    public static function email($data)
    {
        if (!empty($data)) {
            if (filter_var($data, FILTER_VALIDATE_EMAIL) == false) {
                throw new \Sendit\Bliskapaczka\ApiClient\Exception('Invalid email', 1);
            }
        }
    ...
    public static function phone($data)
    {
        if (!empty($data)) {
            ...
            if (!is_array($phoneNumberMatches) || count($phoneNumberMatches) == 0) {
                throw new \Sendit\Bliskapaczka\ApiClient\Exception('Invalid phone number', 1);
            }
        }
    ...
    public static function postCode($data)
    {
        if (!empty($data)) {
            ...
            if (!is_array($matches) || count($matches) == 0) {
                throw new \Sendit\Bliskapaczka\ApiClient\Exception('Invalid post code', 1);
            }
        }
    ...
    public static function parcel($data)
    {
        ...
        foreach ($dimensions as $dimension) {
            if ($data['dimensions'][$dimension] <= 0) {
                throw new \Sendit\Bliskapaczka\ApiClient\Exception('Dimesnion must be greater than 0', 1);
            }
        }
    }
}
