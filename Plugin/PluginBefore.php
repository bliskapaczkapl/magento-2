    ...
    public function beforePushButtons(
        ...
        if($this->_request->getFullActionName() == 'sales_order_view'){
            $buttonList->add(
                ...
                ['label' => __('List przewozowy'), 'onclick' => sprintf("setLocation('%s')", $waybillUrl), 'class' => 'reset'],
                -1
            );
            $buttonList->add(
                'delete_bliska_paczka',
                ['label' => __('Anuluj Bliską paczkę'), 'onclick' => sprintf("setLocation('%s')", $cancelUrl), 'class' => 'reset'],
                -1
            );
            $buttonList->add(
                'advice_bliska_paczka',
                ['label' => __('Awizuj Bliską paczkę'), 'onclick' => sprintf("setLocation('%s')", $adviceUrl), 'class' => 'reset'],
                -1
            );
            $buttonList->add(
                'retry_bliska_paczka',
                ['label' => __('Ponów Bliską paczkę'), 'onclick' => sprintf("setLocation('%s')", $retryUrl), 'class' => 'reset'],
                -1
            );
        }
